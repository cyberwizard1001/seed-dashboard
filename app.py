...
def fetch_Anthropometric():
    ...
    food_df = pd.DataFrame.from_dict(data_dict)
    food_df.to_csv(
        "/tmp/AnthropometricParameters.csv",
        index=False,
        header=[
            "Aadhaar",
            "Month",
...
def fetch_Biochemical():
    ...
    food_df = pd.DataFrame.from_dict(data_dict)
    food_df.to_csv(
        "/tmp/BiochemicalParameters.csv", index=False, header=["Aadhaar", "Month", "Haemoglobin"]
    )
...
ANTHRO_URL = "/tmp/AnthropometricParameters.csv"
BIO_URL = "/tmp/BiochemicalParameters.csv"
CLINIC_URL = "/tmp/ClinicalParameters.csv"
DIET_URL = "/tmp/FoodGroups.csv"

st.markdown(
    "<h1 style='text-align: center; color: red;'>SEED Project - Admin Dashboard</h1>",
    unsafe_allow_html=True,
)
st.sidebar.title("Admin Dashboard")
st.markdown(
    "<div align='center'>Dashboard to gain insights from the user data</div>",
    unsafe_allow_html=True,
)
...
option = st.sidebar.selectbox("Choose an option", dropdown)
...
if option == "Show data":
    ...
    st.markdown(
        "The following table gives you a real-time feed of the AnthropometricParameters table"
    )
    st.dataframe(anthro)
    ...
    fig = px.pie(
        values=anthro["Gender"].value_counts().values, names=anthro["Gender"].value_counts().index
    )
    st.plotly_chart(fig)
